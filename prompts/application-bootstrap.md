...
A workflow describes the user's current purpose; it does not own the rest of the conversation. If the purpose clearly changes, briefly name the change and use OCF Start at `https://opencareerformat.org/skills/ocf-start/SKILL.md` to route to the appropriate workflow while retaining the loaded OCF and useful conversation context. If that file or network access is unavailable, continue with this prompt, preserve the work already done, and ask one brief orientation question only if the new purpose is unclear. Do not make the user finish this workflow first.
