...
LinkedIn and targeted resumes do not need identical wording, detail, or emphasis, but they should fit together as broader and narrower views of the same underlying work. A targeted resume may make one defensible aspect of a general public description more specific; it should not quietly repair or contradict the public version. When two versions no longer fit together, return to the canonical facts and clarify the relationship before exporting either one.

External claims should also pass the **former-colleague test**. Imagine the output being read by someone who understands the organization, the work, and its professional culture. A defensible version should leave them thinking, "I might describe it differently, but that is a fair account of what they did." Different terminology or vantage points are not failures. If the wording instead implies authority, ownership, seniority, exclusivity, or scope the person did not have, narrow the title, statement, or attribution.
...
**Metrics are structured.** When an achievement has numbers — revenue, headcount, percentage improvement — the app captures them as structured data, not just text. This means a curator can compare and rank your achievements by impact, not just display them. A metric makes a number queryable; it does not independently establish the number's evidence or review state. When a quantitative or materially consequential detail needs its own source, confidence, or review decision, preserve it as a `supportingFact`, optionally with a nested metric.
...
Stable `id` fields belong to OCF items: source artifacts, experience entries, positions, achievements, reflections, and other records that a tool may need to edit or reference later. Human-readable slug IDs are recommended for hand-authored files because they make reviews and diffs easier: `old-resume-2024-03`, `northstar-onboarding-compression`, `meridian-proudest-of-2024`. They also give native editors a compact, recognizable name for an achievement whose canonical `statement` may be a full resume bullet point. In a candidate-owned workflow, the editor is usually the subject and often needs only a familiar handle such as `mhs-ransomware-2024`; the full statement remains available when more context is needed. Editors may humanize the ID for display, but references remain bound to the unchanged ID. Do not repurpose `shortStatement` as an internal name: it is alternate outward-facing wording. UUIDs are also acceptable when collision safety matters more than readability. The LLM, importer, or editor does not become the item ID; it belongs in `provenance` as the tool or source that created, imported, or modified the item. This keeps update proposals precise: "add a narrative variant to achievement `northstar-onboarding-compression`" is safer than "edit the onboarding bullet in the first Northstar role."

`reviewStatus` records whether durable, user-facing or user-quoted material is safe to use in curation. Typical values are `unreviewed`, `needs-review`, `user-confirmed`, `do-not-use`, and `superseded`. Tools should treat a missing `reviewStatus` on imported, inferred, or LLM-mined durable items as `unreviewed` until the user confirms otherwise. `user-confirmed` means the person accepted that material for use; it does not mean an employer, colleague, institution, or other source independently corroborated it. Do not overload review status with evidence strength; provenance, confidence, source artifacts, and open questions carry the evidence context.

A review status applies to the item on which it appears. It covers that item's direct content, including fields such as `statement`, `longform`, and metrics that do not define a separate review status. It does not propagate across references, lineage, parent-child relationships, or audience variants, and it does not override a nested record's own review status. Confirming one narrative variant therefore does not confirm its parent achievement, sibling variants, or details found only in the parent's longform.

Downstream tools should reuse explicit claim-bearing material: the selected statement or narrative variant, or a reviewed supporting fact. They should not turn contextual detail found only in `longform`, reflections, notes, or source text into a stronger standalone claim. If a detail is useful enough to reuse independently, first preserve and review it as a supporting fact. This keeps selection, user approval, and evidentiary context from being inferred across fields.
