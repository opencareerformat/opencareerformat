...
- `spec/examples/maria-reyes/implementation-details.md` and `spec/examples/maria-reyes/maria-reyes-revision-7.ocf.json`, including migration from the incubating `extensions.user.local.candidateTalkingPoints` shape now that `talkingPoints` has graduated, and removal of walkthrough language that treats the first file as an imported starter;
...
- Treat `reviewStatus`, `voice`, `cautions`, and `aiInstructions.text` as curation inputs; exporters should not use unreviewed or tool-assembled positioning variants without subject confirmation.
