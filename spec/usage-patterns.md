...
The dependency-free [`reference/context/ocf-context.js`](../reference/context/ocf-context.js) script demonstrates this boundary. It uses a local, editable profile to produce an explicit non-authoritative envelope and can retrieve one full item by its file-local stable ID. Its context profile is reference-tool configuration, not part of the OCF schema, and context reduction is not a substitute for visibility filtering or curation. See [Selective Context Without Splitting the Master](context-profiles.md) for the rationale and limits of the experiment.
