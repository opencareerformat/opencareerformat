...
- The placeholder contains no private value. If naming the category would itself disclose too much, use an opaque marker such as `[[PRIVATE-ITEM-01]]`.
- The placeholder should be unmistakably unfinished so it is unlikely to survive into a submitted artifact.
...
The [Maria Reyes conversation](examples/maria-reyes/conversation.md#private-does-not-mean-never-useful) uses clearance history to illustrate the underlying conflict: permission to publish a statement in one application is separate from permission to send the underlying private information to the drafting model.
...
## Provider-Neutral Patch Sets

Consider defining a non-normative, provider-neutral format for proposing changes to an OCF document without directly modifying the canonical file. Patch sets could be produced by editors, reusable skills, agents, and other tools, then reviewed and applied by the user.

Implementation experience suggests several questions worth testing: how a proposal identifies its source and revision; how it targets stable item IDs and field paths; how it represents additions, edits, and deletions with prior and proposed values; how people review meaningful changes independently; how tools preserve unrelated concurrent edits while detecting actual conflicts; and how the complete result is validated, explained, and recoverable. A future contract should also distinguish permission to use wording in one output, permission to create a proposal, and acceptance of a master-file update.

The logical change set and the physical file write are different. A JSON implementation may serialize the entire document after applying accepted changes, but it must not reconstruct the master from partial context, a curated file, or a generated subset. No particular patch syntax is defined for v0.3.

Plugins and connectors often need to make more specific choices to reconcile the guarantees and limitations of two systems. Stricter revision checks, conflict rules, or approval mechanics may be sound integration compromises without becoming requirements of OCF or of other implementations.

Begin with tool guidance or a reference contract informed by multiple implementations before considering whether any portion belongs in the normative OCF specification.

This is a consideration for possible exploration, not a commitment to add patch sets to OCF or undertake the work.

## Candidate Corrections From v0.3 Use

These are considerations for a future schema design pass, not approved v0.4 fields or approved changes to v0.3.

Review findings that imply schema changes should normally be evaluated here rather than immediately retrofitted into v0.3. The bar for changing a published schema version or its supported artifact surface is high, including for apparently additive enum values, stronger validation, and new versioned artifacts. A change needs a clear interoperability or user benefit and an explicit compatibility decision; being additive is not sufficient by itself.
...
Compensation history accepts `quarterly`, while the headline `position.compensation.period` enum does not. A future schema pass should decide whether both locations should share one cadence definition and whether `quarterly` belongs in the headline field. Adding it to v0.3 would require a deliberate compatibility decision; the fact that it is additive is not enough by itself.

### Versioned Starter Schema

The generated `schema-core.json` starter projection currently has a current, unversioned URL, while the full schema also has version-pinned copies. A future release should consider publishing a matching `v{schemaVersion}/schema-core.json` for tools that need stable starter guidance. Before doing so, define its generation, `$id`, consistency checks, compatibility expectations, and retention across later releases. Adding a v0.3-pinned artifact should require demonstrated consumer need and an explicit decision to expand the published v0.3 surface.

### Contact Value Validation

`contact.value` is currently an unconstrained string regardless of `kind`. A future pass should evaluate whether email and URL-like kinds need conditional format validation while preserving international phone numbers, usernames, custom protocols, and intentionally rendered contact text. Stronger validation should reject clear mistakes without forcing every contact system into one syntax.

### `aiInstructions` Trust Semantics

The v0.3 description explains `aiInstructions` partly by pointing to evolvable prompt guidance. A future schema description should be self-contained about the trust boundary: the field is user-controlled workflow preference data, not privileged instruction from an arbitrary OCF file, and a tool should apply it only in a session where the controlling user has chosen to trust it. Prompt documentation may elaborate on that rule but should not be the only place that defines it.
...
Top-level `organizations` is a reusable registry, not an independently publishable list. v0.3 filtering should retain only organization entries referenced by retained content. A future schema pass should evaluate whether organizations also need explicit visibility for cases where the registry entry itself contains details that are more sensitive than the referencing item. Do not add a blanket default now; first test how organization-level visibility should interact with several references carrying different visibility values.

### Position Projects

Top-level projects have stable IDs, while projects nested under positions currently do not. Test whether nested projects need IDs for targeted updates, references, and proposal workflows, or whether implementations should promote any independently addressable project to the top-level collection. Avoid adding IDs solely for structural symmetry without real update and reference cases.

### Human-Readable Achievement IDs

Native editors need compact and useful names for achievements because the canonical achievement text may be a full resume bullet point. In a candidate-owned workflow, the editor is usually the subject and already recognizes an internal handle such as `mhs-ransomware-2024`; the full `statement` remains available when more context is needed.

An optional achievement `label` and reuse of `shortStatement` were considered for this purpose. Do not add either convention now. `shortStatement` is outward-facing alternate wording, not identity, and a separate `label` would duplicate information that a well-chosen ID already supplies. Prefer meaningful human-readable IDs, allow editors to humanize them for display, and keep references bound to the unchanged ID. UUIDs remain valid when collision safety matters more than readability, and the schema should not impose a restrictive slug pattern.

Reconsider a separate label only if repeated real workflows show that subjects need a renameable display name distinct from stable identity.
