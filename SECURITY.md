...
Generic filtering must also fail closed for opaque containers. A schema-recognized structural path may be traversed without its own visibility field, but an unknown extension namespace must carry explicit valid visibility before a generic derived projection retains it. Editors should still preserve unknown extensions during round-trip; preservation in the private source and disclosure in an output are different operations.
...
The intended audience for an artifact and the model used to draft it are separate trust decisions. A private item may be appropriate in one user-approved application without being appropriate to send to an externally hosted LLM. Users can instead provide a minimal curated statement, use a local model, or add the approved wording after drafting. The [Maria Reyes conversation](spec/examples/maria-reyes/conversation.md#private-does-not-mean-never-useful) uses clearance history to illustrate this conflict.
